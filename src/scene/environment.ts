...
export class EnvironmentView {
  ...
  /** peopleZOffsetM: 人物の前後位置をまとめてずらす(+で正面壁から離れる)。既定0で従来配置 */
  constructor(private scene: any, private peopleZOffsetM = 0) {
  ...
  update(state: AppState) {
    ...
    for (const [x,h,z] of specs) { const person = new THREE.Group(); const mat = new THREE.MeshBasicMaterial({color:0x030609}); const body = new THREE.Mesh(new THREE.CapsuleGeometry(.12, Math.max(.25,h-.38), 4, 8), mat); body.position.y=(h-.18)/2; const head = new THREE.Mesh(new THREE.SphereGeometry(.12,12,8),mat); head.position.y=h-.12; person.add(body,head); person.position.set((x-.5)*state.params.W/1000,0,z*state.params.D/1000+this.peopleZOffsetM); this.people.add(person); }
  }
